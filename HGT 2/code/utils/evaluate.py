...
def evaluate(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd, isTest=True):
    ...
    # خوشه‌بندی و ارزیابی با KMeans
    kmeans = KMeans(n_clusters=nb_classes)
    pred_labels = kmeans.fit_predict(test_embs.cpu().numpy())
    nmi = normalized_mutual_info_score(test_lbls.cpu().numpy(), pred_labels)
    ari = adjusted_rand_score(test_lbls.cpu().numpy(), pred_labels)
    print("Clustering: NMI: {:.4f}, ARI: {:.4f}".format(nmi, ari))

    # نمایش گرافیکی خوشه‌بندی با t-SNE
    tsne = TSNE(n_components=2)
    tsne_embs = tsne.fit_transform(test_embs.cpu().numpy())
    plt.scatter(tsne_embs[:, 0], tsne_embs[:, 1], c=pred_labels, cmap='tab10')
    plt.title("t-SNE Visualization of Node Embeddings")
    plt.show()

    # ارزیابی طبقه‌بندی با Logistic Regression
    accs = []
    micro_f1s = []
    macro_f1s = []
    macro_f1s_val = []
