...
def load_acm(ratio, type_num):
    ...
    adj = sp.load_npz(path + "adj.npz")  # بارگذاری ماتریس مجاورت گراف چندبخشی
    ...
    adj = sparse_mx_to_torch_sparse_tensor(normalize_adj(adj))  # تبدیل ماتریس مجاورت به تانسور
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_a, nei_s], [feat_p, feat_a, feat_s], adj, pos, label, train, val, test
...
def load_dblp(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_p], [feat_a, feat_p], adj, pos, label, train, val, test
...
def load_imdb(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_d, nei_a], [feat_m, feat_d, feat_a], adj, pos, label, train, val, test
